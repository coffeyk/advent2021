...
class Line(NamedTuple):
    ...
    def category(self) -> LineType:
        """category identifies what of line this is."""
        p1 = self.p1
        p2 = self.p2
        if p1 == p2:
    ...
    def walk(self) -> Iterable[Point]:
        """walk iterates over all the valid points in a line."""
        p1 = self.p1
        p2 = self.p2
...
def process_data(data: Iterable[Line]) -> int:
    # Accept all kinds of lines
    line_filter = (LineType.HORIZONTAL, LineType.VERTICAL, LineType.OTHER)
    valid_lines = (l for l in data if l.category() in line_filter)

    # Map of lines that touch a given point
    grid: dict[Point, int] = defaultdict(int)
    for l in valid_lines:
        for p in l.walk():
            grid[p] += 1

    # Count the points with more than one line
    result = 0
